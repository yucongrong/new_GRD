...
from project_utils import named_apply, init_weights_vit, PatchEmbed
from layers import RopePositionEmbedding, SelfAttentionBlock
from config import dtype_dict, norm_layer_dict, ffn_layer_dict, device
...
class DinoVisionTransformer(nn.Module):
    ...
    def forward_features_list(self, x_list: List[Tensor], masks_list: List[Tensor]) -> List[Dict[str, Tensor]]:
        ...
        for blk in self.blocks:
            if hasattr(blk.attn, 'visualize_images'):
                blk.attn.visualize_images = None
                blk.attn.visualize_images = self.visualize_images
        for idx, blk in enumerate(self.blocks):
            if self.rope_embed is not None:
                rope_sincos = [self.rope_embed(H=H, W=W) for H, W in rope]
            else:
                rope_sincos = [None for r in rope]
            x = blk(x, rope_sincos, layer_idx=idx)
        all_x = x
        output = []
    ...
    def forward(self, *args, is_training: bool = False, **kwargs) -> List[Dict[str, Tensor]] | Tensor:
        original_imgs = kwargs.pop("original_imgs", None)
        for blk in self.blocks:
            if hasattr(blk.attn, 'visualize_images'):
                blk.attn.visualize_images = None
        self.visualize_images = original_imgs
        for blk in self.blocks:
            if hasattr(blk.attn, 'visualize_images'):
                blk.attn.visualize_images = original_imgs
...
class ImageDataset(Dataset):
    def __init__(self, filename_list, root_dir, transform, full_paths=None):
